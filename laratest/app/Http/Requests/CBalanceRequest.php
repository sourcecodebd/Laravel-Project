...
class CBalanceRequest extends FormRequest
{
    /**
    ...
    {
        return [
            'username' => 'required|unique:customer_balance|min:3|max:30|regex:/[a-zA-Z0-9]/i',
            'card_no' => 'required|digits:5',
            'email' => 'required|email:rfc|max:50|min:10',
            'bank_name' =>'required|min:5|max:30|regex:/[a-zA-Z]/i' ,
            'add' => 'required|min:3|max:10|regex:/^\d+(\.\d{1,2})?$/',
    ...
    public function messages()
    {
        return [
            ...
            'add.required' => "Added Amount can't be empty...",
            'add.min' => "Added Amount must be minimum 3 digits...",
            'add.max' => "Added Amount can't exceed 10 digits...",
            'add.regex' => "Added Amount must be decimal value...",
